...
- **Agy IDE MCP Config:** Agy IDE (Antigravity) does NOT read MCP servers from `settings.json` or `mcp.json`. It strictly uses `C:\Users\Felix\.gemini\config\mcp_config.json`. Furthermore, each MCP server entry MUST include the attribute `"$typeName": "exa.cascade_plugins_pb.CascadePluginCommandTemplate"` to load successfully.
