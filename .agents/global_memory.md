...
- **MCP State Inconsistency:** If an MCP server returns unexpected empty data (e.g., `aio-agentic-sdlc` returning an empty backlog), fallback to the native CLI equivalent (e.g., `uvx aio-agentic-sdlc next`), as the CLI may have correct access to local state files.
...
- **Bootstrapping Git Tags for Auto-Versioning:** If you must programmatically create an initial `v0.0.0` tag in a CI/CD workflow to unblock auto-versioning tools (like `github-tag-action`), you MUST attach the tag to the *first commit* of the repository (`git tag v0.0.0 $(git rev-list --max-parents=0 HEAD | tail -1)`). If you tag `HEAD`, the versioning tool will see 0 commits since the tag and fail to trigger a release.
