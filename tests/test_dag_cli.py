...
def sample_dag_file(tmp_path):
    ...
    node1 = Node(id="n1", type=NodeType.SYSTEM, name="System 1", domain="core")
    node2 = Node(id="n2", type=NodeType.SYSTEM, name="System 2", domain="core")
...
def test_cli_node_add(sample_dag_file):
    ...
    result = runner.invoke(cli, [
        ...
        "--id", "n3", "--type", "module", "--name", "Module 3", "--domain", "users"
    ])
    assert result.exit_code == 0
    
    manager = DAGManager.load(sample_dag_file)
    assert "n3" in manager.nodes
    assert manager.get_node("n3").type == NodeType.MODULE
    assert manager.get_node("n3").name == "Module 3"

def test_cli_node_update(sample_dag_file):
    runner = CliRunner()
    result = runner.invoke(cli, [
        "node", "update", "--file", sample_dag_file, 
        "--id", "n1", "--description", "Updated desc"
    ])
    assert result.exit_code == 0
    
    manager = DAGManager.load(sample_dag_file)
    assert manager.get_node("n1").description == "Updated desc"

def test_cli_node_remove(sample_dag_file):
    runner = CliRunner()
    result = runner.invoke(cli, [
        "node", "remove", "--file", sample_dag_file, "--id", "n2"
    ])
    assert result.exit_code == 0
    
    manager = DAGManager.load(sample_dag_file)
    assert "n2" not in manager.nodes
...
def test_cli_edge_add(sample_dag_file):
    ...
    assert manager.edges[1].source == "n2"
